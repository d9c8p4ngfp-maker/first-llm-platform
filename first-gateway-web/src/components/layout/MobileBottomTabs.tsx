...
export function MobileBottomTabs() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 flex border-t border-[hsl(var(--border))] bg-[hsl(var(--background))] pb-[env(safe-area-inset-bottom)] md:hidden">
      {tabs.map(({ to, label, icon: Icon }) => (
        <Link
          ...
          className={cn(
            ...
            isActive(to) ? 'text-[hsl(var(--foreground))]' : 'text-neutral-500',
          )}
        >
