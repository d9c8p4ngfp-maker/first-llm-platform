...
export function ScheduleMarquee({ items, emptyLabel, expanded = false }: ScheduleMarqueeProps) {
  ...
  useEffect(() => {
    if (expanded || labels.length <= 1 || paused) return undefined
    const timer = window.setInterval(() => {
      setIndex((prev) => (prev + 1) % labels.length)
    }, TICK_MS)
    return () => window.clearInterval(timer)
  }, [expanded, labels.length, paused])
  ...
  return (
    <span
      className="group/marquee relative block h-5 min-w-0 flex-1 overflow-hidden"
      aria-live="polite"
    ...
    >
      <span
        className="block transition-transform duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] motion-reduce:transition-none"
        style={{ transform: `translateY(calc(-1 * ${safeIndex} * ${LINE_HEIGHT}))` }}
      >
        {labels.map((label, i) => (
          <span
            key={`${items[i]?.id ?? i}-${label}`}
            className="block h-5 leading-5 truncate whitespace-nowrap text-[hsl(var(--foreground))] group-hover/marquee:text-brand motion-reduce:group-hover/marquee:text-[hsl(var(--foreground))]"
          >
            {label}
          </span>
        ))}
      ...
      {labels.length > 1 && (
        <span
          className="pointer-events-none absolute bottom-0 right-0 top-0 w-8 bg-gradient-to-l from-[hsl(var(--card))] to-transparent opacity-0 transition-opacity group-hover/marquee:opacity-100"
          aria-hidden
        />
      )}
    </span>
