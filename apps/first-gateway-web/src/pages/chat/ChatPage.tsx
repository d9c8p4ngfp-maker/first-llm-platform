...
export function ChatPage() {
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-10rem)] max-w-6xl gap-4 md:h-[calc(100vh-11rem)]">
      ...
      <div className="flex flex-1 flex-col overflow-hidden rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-console">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-4">
          {messages.length === 0 && !streaming && (
            <div className="empty-chat-glow flex h-full flex-col items-center justify-center text-center text-[hsl(var(--muted-foreground))]">
              <p className="font-mono-brand text-xs uppercase tracking-[0.2em] text-brand">Ready</p>
              <p className="mt-3 font-display text-xl font-semibold text-[hsl(var(--foreground))]">开始对话</p>
              <p className="mt-2 max-w-sm text-sm">选择模型或 Skill，在下方输入第一条消息</p>
            </div>
          )}
          {messages.map((m) => (
          ...
          {streaming && !streamText && (
            <div className="flex max-w-[85%] items-center gap-2 rounded-2xl border border-brand/20 bg-brand-muted/30 px-4 py-3 text-sm text-[hsl(var(--muted-foreground))]">
